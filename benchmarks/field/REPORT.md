...
| 3 — table columns | **open, and one attempt is now measured.** A second evidence unit per CELL (`row · column: value`) was built so a correctly bound unit could compete with the mis-ordered flattened line; the probe above answers `12 hours` with it and without it, because pdfplumber never hands `learn_rows` a clean row for that table — the loss is upstream of the index. Reverted. | COST.md §8.4 |
| 4 — two-row spreadsheet headers | **split in two; the row-label half is closed.** The leading dot of `.Puerto Rico` is indentation, and the plain name is now an alias of the row — that question, refused at every commit in this history, is now answered with no model call (5x10x3 median: census 9/10 -> 10/10, zero-call 6 -> 7). The header-block half is now CLOSED TOO, one turn later: reading the block off the sheet's own merges recovers 2021-2023 and measured 10/10 -> 6/10 with two wrong answers, because "population estimate as of july 1" then had four equally good answers and the path chose one silently. The choosing was the defect; `lookup` now answers such a question with ALL the records that reading names, each under the sheet's own column name, and both halves ship on: census 10/14 -> 14/14 with 0 wrong (four year-naming questions added), field total 53/54. | `core/dataset.bare`, `tables._header_names`, `lookup._reading`, `LMM_XLSX_HEADER_BLOCK`, `LMM_LOOKUP_CANDIDATES`, tests R1/R2/N8/N9, COST.md §8.4.1 |
| 6 — no language gate, 9-11 | **open**, and listed in the README's honest limits. |  |
