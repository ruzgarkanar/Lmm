...
| 4 — two-row spreadsheet headers | **split in two; the row-label half is closed.** The leading dot of `.Puerto Rico` is indentation, and the plain name is now an alias of the row — that question, refused at every commit in this history, is now answered with no model call (5x10x3 median: census 9/10 -> 10/10, zero-call 6 -> 7). The header-block half is BUILT AND OFF: reading the block off the sheet's own merges recovers the 2021 and 2022 columns and measured 10/10 -> 6/10 with two wrong answers, because "population estimate as of july 1" then has four equally good answers. | `core/dataset.bare`, `tables._header_names`, `LMM_XLSX_HEADER_BLOCK`, tests R1/R2, COST.md §8.4 |
