...
RESTORED, AND THE REASON IT WAS DELETED WAS WRONG. This file was
retired as "an idle derivation loop on no product path". It is on the
product path: `lmm/chat.py` — the `lmm` console command declared in
pyproject — builds a `Mind` at startup and calls it on every turn, and
it drives `?wonder`, `?research <topic>` and the between-turn
derivation. The deletion shipped a CLI that crashed on launch, and
every test still passed, because the import sits inside `main()`.
A claim about what nothing uses is checkable; that one was not checked,
and W125 checks it now.
"""
